...
class word
{
  ...
  public:
    ...
    // Convert word as boolean to a mask with all zeroes (false) or all ones (true), but negating it.
    ...
    friend word operator-(word const& w, int value)
    {
      ...
      assert((w.M_word & (w.M_word - 1)) == 0);		// Must be a power of 2.
      assert(value == 1);				// May only substract 1.
      return word(w.M_word - 1);
    }
