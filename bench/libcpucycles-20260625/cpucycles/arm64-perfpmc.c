...
// 20251226 djb: wrap inside perf_event_open (otherwise current kernels disable pmccntr if another process uses perf)
// 20251226 djb: forked from arm64-pmc.c
  ...
  if (fdperf == -1) {
    ...
    for (xkernel = 1;xkernel >= 0;--xkernel) {
      for (xhv = 1;xhv >= 0;--xhv) {
        ...
        attr.exclude_hv = xhv;
        attr.config1 = 3; // 1 (user enable) + 2 (64-bit)
